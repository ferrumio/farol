...
#[cfg(test)]
mod tests {
    //! These tests exercise private helpers (`extract_region`, `dedent`,
    //! `extract_lines`) that are deliberately not part of the public API,
    //! so they stay in-file. Public-surface tests live in
    //! `tests/plugins_code_include.rs`.
    use super::*;
